...
void donus(uint32_t donusAralik) {
  if (millis() - sonDonus > donusAralik*saniye) {
    myServo.write(30);
    digitalWrite(ledR, 1);
    delay(1500);
    myServo.write(90);
...
void sicaklikAyar() {
  ...
  while (true) {
    ...
    if (digitalRead(bYukari)) {
      sicaklikDegeri = constrain(sicaklikDegeri + 0.1, 20.0, 40.0);
      delay(50);
...
void cevirmeAyar() {
  ...
  while (true) {
    lcd.home();
    lcd.print("Eski sure:"); lcd.print(eepromDonus); lcd.print(" sn");
    lcd.setCursor(0,1); 
    lcd.print("Yeni sure:"); lcd.print(dondurme); lcd.print(" sn");

    if (digitalRead(bYukari)) {
      while(digitalRead(bYukari));
      dondurme = constrain(dondurme + 1, 5, 30);
      delay(50);
    } else if (digitalRead(bAsagi)) {
